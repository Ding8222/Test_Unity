...
public class NodeEditor : EditorWindow {
    ...
    void DrawNodeWindow(int id)
    {
        ...
        else if(node.ToolBarSelectIndex == 1)
        {
            ...
            node.SubTreeAsset = EditorGUILayout.ObjectField("SubTree TextAsset", node.SubTreeAsset, typeof(TextAsset), false) as TextAsset;
            if (node.SubTreeAsset != null && GUILayout.Button("Add SubTree"))
            {
                if(node.SubTreeAsset.text.Length == 0)
                    node.AddNode(NodeGraph.CreateFromBinary(node.SubTreeAsset.bytes), new Vector2(200, 0));
                else
